                ...
                @else
                    <ul class="space-y-4">
                        @foreach ($results as $result)
                            <li class="border border-gray-300 p-4 rounded hover:bg-gray-50 transition duration-200">
                                <!-- Make the result clickable -->
                                <a href="{{ route('result.details', ['surah_id' => $result->surah_id, 'ayah_index' => $result->ayah_index]) }}" class="block">
                                    @if ($searchIn === 'Words')
